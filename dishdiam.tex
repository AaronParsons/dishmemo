...
four to five times brighter that the expected reionization signal \citep{unknown}XXX, so a conservative definition of
$S$ requires
\begin{equation}
...
$\AI$ falls off by -60 dB.  At this level, the contribution from antenna effects are well below the signal level in the region outside from where the foregrounds contaminate.  XXXI added this weak sentence to try and motivate the spec a little better -- needs improvement.XXX
...
brighter as $C_\ell\propto\ell^{-3}$ \citep{unknown}XXX.
...
The equation for sensitivity has been described in previous works ({\em c.f} \cite{2012ApJ...753...81P}, XXX\cite{unknown}) and depends on many parameters
